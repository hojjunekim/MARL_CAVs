...
def init_wandb(args, config, output_dir):
    ...
    # Convert config parser sections to a flat dictionary for logging
    wandb_config = {}
    ...
    run = wandb.init(
        project="mappo-highway-env", # You can change this project name
        config=wandb_config,
        name=output_dir.split('/')[-1], # Use the experiment folder name
        reinit=True
    )
    return run
...
def train(args):
    ...
    # ... (rest of the code to determine model_dir remains unchanged) ...
    ...
    while mappo.n_episodes < MAX_EPISODES:
        ...
        if mappo.episode_done and ((mappo.n_episodes + 1) % EVAL_INTERVAL == 0):
            ...
            # 2. Log Evaluation Metrics to W&B
            wandb.log({
